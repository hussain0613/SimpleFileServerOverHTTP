...
def get_contents():
    ...
    data = {"current_directory": dir_path, "parent_directory": os.path.dirname(dir_path)}
    #try:
    fns = os.listdir(cdir) #folder or file names
    for fn in fns:
        details = {"name": fn}
        real_path: str = os.path.join(os.path.join(cdir, fn))
        ...
        details["time"] = datetime.datetime.fromtimestamp(stats.st_ctime)     

        data[os.path.join(dir_path, fn)] = details # key is the relative path
